...
import { Plus, Trash2, Save, Printer, FilePlus, Pencil, Trash, Eye, RefreshCw } from "lucide-react";
...
export function GRCreatePage() {
  ...
  const addLineItem = () => setLineItems([...lineItems, { kode_Brg: "", model: "", jumlah: 1, serialNo: "", harga: 0, discPct: 0, disc: 0, total: 0, description: "", kode_Gudang: "", information: "" }]);
  ...
  return (
    <div className="space-y-4 p-4 lg:p-6">
      <div>
        <h1 className="text-2xl font-semibold tracking-tight">Goods Receipt (GR)</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="flex flex-wrap items-center gap-2 rounded-md border bg-card p-2">
          ...
          <Button type="submit" variant="ghost" size="sm" className="gap-1.5" disabled={createGR.isPending}><Save className="size-4" />Save</Button>
          <Button type="button" variant="ghost" size="sm" className="gap-1.5"><Printer className="size-4" />Print</Button>
          <Button type="button" variant="ghost" size="sm" className="gap-1.5"><RefreshCw className="size-4" />ReSync</Button>
          <Button type="button" variant="ghost" size="sm" className="gap-1.5 text-destructive"><RefreshCw className="size-4" />Sync to GGMC</Button>
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <div className="space-y-1.5">
              ...
              <DataSelect items={confirmedPOItems} value={doku_PO} onValueChange={(v) => {
                ...
                if (po?.tgl) setTgl_PO(po.tgl.split("T")[0]);
              }} placeholder="Select PO" />
            </div>
            <div className="space-y-1.5">
              <label className="text-xs font-medium">PO Date</label>
              <Input type="date" value={tgl_PO} onChange={(e) => setTgl_PO(e.target.value)} />
            </div>
            <div className="space-y-1.5">
              <label className="text-xs font-medium">DO Vendor</label>
              <Input value={suratJalan} onChange={(e) => setSuratJalan(e.target.value)} placeholder="Delivery note" />
            </div>
            <div className="space-y-1.5">
              <label className="text-xs font-medium">DO Date</label>
              <Input type="date" value={tglSuratJalan} onChange={(e) => setTglSuratJalan(e.target.value)} />
            </div>
            <div className="space-y-1.5">
              <label className="text-xs font-medium">Nopen</label>
              <Input value={nopen} onChange={(e) => setNopen(e.target.value)} placeholder="Customs ref" />
            </div>
            <div className="space-y-1.5">
              <label className="text-xs font-medium">Nopen Date</label>
              <Input type="date" value={tglNopen} onChange={(e) => setTglNopen(e.target.value)} />
            </div>
            <div className="space-y-1.5">
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-medium">AWB/BL</label>
              <Input value={awbBl} onChange={(e) => setAwbBl(e.target.value)} placeholder="Airway/Bill of Lading" />
            </div>
          </div>
        </div>
        ...
        <div className="rounded-md border bg-card">
          <div className="flex items-center justify-between border-b p-3">
            <h2 className="text-sm font-semibold uppercase tracking-wider text-muted-foreground">Item Detail</h2>
                ...
                {lineItems.length === 0 ? (
                  ...
                  lineItems.map((item, index) => (
                    <TableRow key={index}>
                      ...
                      <TableCell><Input value={item.serialNo} onChange={(e) => updateLineItem(index, "serialNo", e.target.value)} className="h-8" /></TableCell>
                      ...
                      <TableCell><Input value={item.description} onChange={(e) => updateLineItem(index, "description", e.target.value)} className="h-8" /></TableCell>
                      <TableCell>
                        <DataSelect items={warehouseItems} value={item.kode_Gudang} onValueChange={(v) => updateLineItem(index, "kode_Gudang", v)} placeholder="WH" />
                      </TableCell>
                      <TableCell><Input value={item.information} onChange={(e) => updateLineItem(index, "information", e.target.value)} className="h-8" /></TableCell>
                      <TableCell>
        ...
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
          <div className="space-y-3 rounded-md border bg-card p-4">
            <h2 className="text-sm font-semibold uppercase tracking-wider text-muted-foreground">Info & Notes</h2>
            <div className="space-y-1.5">
              <label className="text-xs font-medium">Note</label>
              <Textarea value={memo} onChange={(e) => setMemo(e.target.value)} rows={2} placeholder="Notes..." />
            </div>
            <div className="space-y-1.5">
              <label className="text-xs font-medium">Forward Agent</label>
              <Input value={forwardAgent} onChange={(e) => setForwardAgent(e.target.value)} placeholder="Forwarding agent" />
            </div>
            <Button type="button" variant="outline" size="sm"><Printer className="mr-1.5 size-3.5" />Import WMS Excel</Button>
          </div>
          <div className="rounded-md border bg-card p-4">
